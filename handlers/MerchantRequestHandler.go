...
func (env Env) ReverseCapture(w http.ResponseWriter, r *http.Request) {
	...
	handleSuccessResponse(fmt.Sprintf("The transaction with ref %s is reversed and can not be charge back to the card", transReq.transactionId), w)
}
...
func (env Env) CaptureMoney(w http.ResponseWriter, r *http.Request) {
	...
	handleSuccessResponse("The amount is capture and sent to merchant", w)
}
...
func (env Env) Refund(w http.ResponseWriter, r *http.Request) {
	...
	handleSuccessResponse(fmt.Sprintf("The transaction with ref %s has been refund", refundReq.transactionId), w)
}
